...
const Conversations: FC<Props> = ({
  ...
  const checkedOutInteraction = useMemo(
    () =>
      appDatas?.find(
        (appData) =>
          appData?.data?.exchanges?.exchangesList &&
      ...
      <Box py={2} px={20} border="1px solid #ccc" borderRadius="8px">
        {checkedOutInteraction ? (
          <Stack spacing={2}>
            <MessagesPane
              currentExchange={
                checkedOutInteraction.exchanges.exchangesList[
              ...
              pastMessages={
                checkedOutInteraction.exchanges.exchangesList.flatMap(
                  (exchange) => {
              ...
              <IconButton
                ...
                onClick={() =>
                  deleteAppData({
                    id:
                      appDatas?.find(
                        (appData) => appData.member.id === checkedOutMember.id,
                      )?.id || '',
                  })
                }
                sx={{ width: 'auto' }}
